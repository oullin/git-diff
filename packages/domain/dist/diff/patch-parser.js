...
export function parseHunkHeader(text) {
...
export function getHunkInfos(lines) {
    const infos = [];
    let prevOldEnd = 0;
    let prevNewEnd = 0;
...
export function parsePatch(section, hideWhitespace) {
    if (section.binary) {
...
function parseViaPierre(section) {
    let parsed;
    try {
    ...
    const lines = [];
    let counter = 0;
    const idFor = (suffix) => `${section.id}:p${counter++}:${suffix}`;
...
function parseLineByLine(section) {
    const lines = [];
    let oldLine = 0;
    let newLine = 0;
...
export function splitPatch(section, hideWhitespace) {
    return splitPatchLines(parsePatch(section, hideWhitespace));
}
export function splitPatchLines(lines) {
    const rows = [];
    let i = 0;
