...
export function renderMarkdownWithLineAnchors(source: string, addedLines: ReadonlySet<number>, renderInline: (markdown: string) => string, sanitize: (html: string) => string): string {
	...
	for (let i = 0; i < sourceLines.length; i++) {
		...
		out.push(`<div class="md-line${addedClass}" data-line="${lineNumber}">${rendered || '&nbsp;'}</div>`);
	}
