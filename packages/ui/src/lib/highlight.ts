...
import { isHighlightPoolAvailable, MAX_LINE_TOKENIZE_LENGTH, prewarmLanguage, requestHighlight, type ThemeId } from '@lib/highlightPool';
...
function tokenizeWithHighlighter(h: HighlighterCore, text: string, lang: BundledLanguage, theme: ThemeId): string {
...
export class Highlighter {
	...
	private scheduleAsync(key: string, text: string, lang: BundledLanguage, theme: ThemeId): void {
		...
		const tokenize = isHighlightPoolAvailable() ? requestHighlight(text, lang, theme) : this.fallbackEnsureLanguage(lang).then((h) => tokenizeWithHighlighter(h, text, lang, theme));
