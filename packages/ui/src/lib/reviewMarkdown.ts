  ...
  switch (tag) {
    ...
    case "ol": {
      ...
      for (const child of Array.from(element.children)) {
        ...
        items.push(`${"  ".repeat(ctx.listDepth)}${marker} ${inner.replace(/\n/g, "\n   ")}`);
        index += 1;
      }
      return items.join("\n") + "\n\n";
    }
...
export function formatReviewAsMarkdown(review: ReviewDetail): string {
  ...
  if (session.contextKind === "commit" && session.contextSha) {
    ...
    if (session.headSha) meta.push(`HEAD: \`${session.headSha}\``);
  }
  ...
  for (const [filePath, comments] of grouped) {
    lines.push(`## \`${filePath}\``);
    lines.push("");
    comments.sort((a, b) => a.lineNumber - b.lineNumber || a.createdAt.localeCompare(b.createdAt));
    for (const comment of comments) {
      const side = comment.side === "del" ? "old" : comment.side === "add" ? "new" : comment.side;
      const author = comment.authorLabel || "reviewer";
      lines.push(`- **L${comment.lineNumber}** (${side}) — _${author}, ${comment.createdAt}_`);
