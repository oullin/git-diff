...
<template>
	<div class="flex h-full min-h-0 flex-col">
		...
		<template v-else>
			<header class="flex items-center justify-between border-b border-border px-4 py-2 text-xs text-muted-foreground">
				<span class="truncate font-mono">{{ path }}</span>
				<span class="ml-3 shrink-0">{{ sizeLabel }}</span>
			</header>

			<div v-if="isImage && imageLoading" class="grid flex-1 place-items-center text-sm text-muted-foreground">
				<div class="flex items-center gap-2">
					<Loader2 class="h-4 w-4 animate-spin" />
					Loading image…
				</div>
			...
			<div v-else-if="isImage && imageError" class="grid flex-1 place-items-center p-8 text-sm text-destructive">
				{{ imageError }}
			</div>

			<div v-else-if="isImage && imageUrl" class="grid flex-1 place-items-center overflow-auto p-4">
				<img :src="imageUrl" :alt="path" :style="{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }" />
			</div>

			<div v-else-if="file.binary" class="grid flex-1 place-items-center p-8 text-sm text-muted-foreground">Binary file ({{ sizeLabel }}) — preview unavailable.</div>

			<div v-else-if="file.truncated" class="grid flex-1 place-items-center p-8 text-sm text-muted-foreground">File is larger than 2 MB ({{ sizeLabel }}) — preview unavailable.</div>

			<pre v-else class="min-h-0 flex-1 overflow-auto bg-panel p-4 font-mono text-xs leading-relaxed"><code>{{ file.content }}</code></pre>
		</template>
	</div>
</template>
