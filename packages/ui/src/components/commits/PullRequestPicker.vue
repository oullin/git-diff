...
<template>
	<Popover v-model:open="open">
		...
		<PopoverContent class="w-[32rem] p-0" align="start">
			<header class="border-b border-border px-3 py-2 text-xs font-semibold text-muted-foreground">Open pull requests</header>
			<div v-if="loading" class="flex items-center justify-center gap-2 py-6 text-xs text-muted-foreground">
				<Loader2 class="h-3.5 w-3.5 animate-spin" />
				<span>Loading…</span>
			</div>
			<ul v-else-if="pullRequests.length > 0" class="max-h-[26rem] overflow-y-auto py-1" role="listbox">
			...
			<div v-else class="px-3 py-6 text-center text-xs text-muted-foreground">No open pull requests.</div>
		</PopoverContent>
	</Popover>
</template>
