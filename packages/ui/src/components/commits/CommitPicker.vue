...
import type { CommitSummary } from '@git-diff/domain';
	...
	<div class="flex items-center gap-1">
		<Popover v-model:open="open">
			<PopoverTrigger as-child>
				<button type="button" class="inline-flex items-center gap-1.5 rounded border border-border bg-background px-2.5 py-1 text-xs font-medium hover:bg-muted">
					<History class="h-3.5 w-3.5" />
					<span>{{ mode === 'commit' ? 'Browse commits' : 'History' }}</span>
				</button>
			</PopoverTrigger>
			<PopoverContent class="w-[28rem] p-0" align="start">
				<header class="border-b border-border px-3 py-2 text-xs font-semibold text-muted-foreground">Recent commits</header>
				<div v-if="loading" class="flex items-center justify-center gap-2 py-6 text-xs text-muted-foreground">
					<Loader2 class="h-3.5 w-3.5 animate-spin" />
					<span>Loading…</span>
				</div>
				<ul v-else-if="commits.length > 0" class="max-h-[26rem] overflow-y-auto py-1" role="listbox">
				...
				<div v-else class="px-3 py-6 text-center text-xs text-muted-foreground">No commits found.</div>
			</PopoverContent>
		</Popover>
		<button
			v-if="mode === 'commit'"
