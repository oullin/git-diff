    ...
    <Dialog :show="open" max-width="lg" @close="close">
        <Command class="bg-popover" :model-value="null">
            <div class="flex items-center border-b border-border px-3">
                ...
                />
                <span
                    v-if="commandBarBinding"
                    class="ml-3 rounded border border-border bg-muted px-1.5 py-0.5 text-xs text-muted-foreground"
                >
                    {{ formatShortcut(commandBarBinding) }}
                </span>
            </div>
            <CommandList class="max-h-80 overflow-y-auto p-1">
                <CommandEmpty class="px-3 py-6 text-center text-sm text-muted-foreground">
