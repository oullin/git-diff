...
async function forgotPassword() {
	const confirmed = window.confirm(`Reset ${props.osUsername}? This deletes the saved password, preferences, and review data for this user. This cannot be undone.`);
				...
				>
					<form class="flex flex-col gap-6" @submit.prevent="submit">
						<div class="flex flex-col gap-1.5">
							<h1 class="text-[22px] font-semibold tracking-tight" :style="{ color: 'var(--gd-text)' }">Welcome back</h1>
							<p class="text-[13.5px] leading-relaxed" :style="{ color: 'var(--gd-text-3)' }">
								Sign in as
								<span class="font-medium" :style="{ color: 'var(--gd-text-2)' }">{{ props.osUsername }}</span>
						...
						<div class="flex flex-col gap-4">
							...
							<label class="flex items-center gap-2 text-[13px]" :style="{ color: 'var(--gd-text-3)' }">
								<input
									v-model="remember"
									type="checkbox"
