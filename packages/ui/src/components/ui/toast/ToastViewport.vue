...
function toneClass(tone: ToastTone = 'info') {
	...
	if (tone === 'error') {
		return 'border-[var(--status-danger-border)]';
	}
...
<template>
	<div v-if="toasts.length" data-testid="toast-viewport" :class="cn('pointer-events-none fixed right-4 bottom-4 z-50 grid justify-items-end gap-2', props.class)">
		<div
			v-for="toast in toasts"
			:key="toast.id"
			:class="cn('pointer-events-auto rounded-md border bg-popover p-3 text-popover-foreground shadow-overlay', toast.wide ? 'w-[32rem]' : 'w-80', toneClass(toast.tone))"
		>
			<div class="flex items-start gap-3">
				...
				<button type="button" class="cursor-pointer rounded-sm p-1 text-muted-foreground hover:bg-muted hover:text-foreground" @click="emit('dismiss', toast.id)">
					<X class="size-3.5" />
