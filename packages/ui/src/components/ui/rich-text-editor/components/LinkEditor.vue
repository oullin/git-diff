...
<template>
	...
	>
		<template v-if="!editing">
			<a :href="url" target="_blank" rel="noopener noreferrer" class="max-w-[18rem] truncate text-primary underline-offset-2 hover:underline">
				{{ url }}
			</a>
			<button type="button" class="ml-1 rounded p-1 hover:bg-accent" :title="'Edit link'" @click="startEdit">
				<Pencil class="h-3.5 w-3.5" />
			</button>
			<button type="button" class="rounded p-1 hover:bg-accent" :title="'Remove link'" @click="removeLink">
		...
		<template v-else>
			...
			/>
			<button type="button" class="rounded p-1 hover:bg-accent" :title="'Apply'" @click="applyEdit">
				<Check class="h-3.5 w-3.5" />
			</button>
			<button type="button" class="rounded p-1 hover:bg-accent" :title="'Cancel'" @click="close">
				<X class="h-3.5 w-3.5" />
			</button>
		</template>
	</div>
