...
<template>
	<Dialog :open="open" @update:open="(val: boolean) => emit('update:open', val)">
		...
		<DialogBody class="space-y-4">
			...
			<div>
				<label class="mb-1 block text-xs font-medium text-muted-foreground">{{ upload ? 'or paste URL' : 'Image URL' }}</label>
				<input
					v-model="url"
					type="url"
					placeholder="https://"
		...
		<DialogFooter>
			...
			<Button :disabled="busy" @click="submit"> <Loader2 v-if="busy" class="mr-2 h-4 w-4 animate-spin" />Insert </Button>
		</DialogFooter>
	</Dialog>
</template>
