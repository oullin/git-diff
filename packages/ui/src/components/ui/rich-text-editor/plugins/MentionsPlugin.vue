...
<template>
    <TypeaheadMenuPlugin
        ...
        @query-change="(value) => (query = value)"
    ...
    >
        <template #default="{ anchorElementRef, itemProps, matchingString }">
            <Teleport v-if="anchorElementRef && options.length > 0" :to="anchorElementRef">
                <div
                    class="z-50 max-h-72 w-56 overflow-y-auto rounded-md border border-border bg-popover p-1 text-sm shadow-md"
                >
                    <button
                    ...
                    <div
                        v-if="itemProps.options.length === 0 && matchingString"
                        class="px-2 py-1.5 text-xs text-muted-foreground"
                    >
                        No matches for "{{ matchingString }}"
                    </div>
                </div>
            </Teleport>
        </template>
