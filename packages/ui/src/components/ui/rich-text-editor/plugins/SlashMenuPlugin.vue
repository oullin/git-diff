...
import { $createParagraphNode, $getSelection, $isRangeSelection, type LexicalEditor, type TextNode } from 'lexical';

import { INSERT_CHECK_LIST_COMMAND, INSERT_ORDERED_LIST_COMMAND, INSERT_UNORDERED_LIST_COMMAND } from '@lexical/list';

import { Code, Heading1, Heading2, Heading3, Image as ImageIcon, List, ListChecks, ListOrdered, Pilcrow, Quote, Table as TableIcon } from 'lucide-vue-next';
...
class SlashOption extends MenuOption {
	...
	constructor(label: string, icon: Component, keywords: string[], perform: (editor: LexicalEditor) => void) {
		super(label);
		this.label = label;
		this.icon = icon;
...
function clearSelectionAndRun(fn: (editor: LexicalEditor) => void): (editor: LexicalEditor) => void {
	return (ed) => fn(ed);
}
...
function filterOptions(query: string | null): SlashOption[] {
	...
	return source.filter((opt) => opt.label.toLowerCase().includes(lower) || opt.keywords.some((kw) => kw.includes(lower)));
}

function triggerFn(text: string): { leadOffset: number; matchingString: string; replaceableString: string } | null {
...
function onSelectOption({ option, closeMenu, textNodeContainingQuery }: { option: SlashOption; closeMenu: () => void; textNodeContainingQuery: TextNode | null; matchingString: string }): void {
	editor.update(() => {
...
<template>
	<TypeaheadMenuPlugin :options="filterOptions(queryString)" :trigger-fn="triggerFn" @query-change="onQueryChange" @select-option="onSelectOption">
		<template #default="{ anchorElementRef, itemProps, matchingString }">
			<Teleport v-if="anchorElementRef" :to="anchorElementRef">
				<div class="z-50 max-h-72 w-60 overflow-y-auto rounded-md border border-border bg-popover p-1 text-sm shadow-md">
					<button
					...
					<div v-if="itemProps.options.length === 0" class="px-2 py-1.5 text-xs text-muted-foreground">No commands matching "{{ matchingString }}"</div>
				</div>
			</Teleport>
		</template>
	</TypeaheadMenuPlugin>
