...
import { Bold, Code as CodeIcon, ImageIcon, Italic, Link as LinkIcon, Redo2, Strikethrough, Table as TableIcon, Underline as UnderlineIcon, Undo2 } from 'lucide-vue-next';
...
<template>
	<div class="flex flex-wrap items-center gap-1 border-b border-border bg-muted/40 px-2 py-1.5" @mousedown.prevent>
		<ToolbarButton :pressed="false" :disabled="!canUndo" title="Undo" @click="editor.dispatchCommand(UNDO_COMMAND, undefined)">
			<Undo2 class="h-4 w-4" />
		</ToolbarButton>
		<ToolbarButton :pressed="false" :disabled="!canRedo" title="Redo" @click="editor.dispatchCommand(REDO_COMMAND, undefined)">
			<Redo2 class="h-4 w-4" />
		</ToolbarButton>
		...
		<ToolbarButton :pressed="state.isUnderline" title="Underline (Ctrl+U)" @click="format('underline')">
			<UnderlineIcon class="h-4 w-4" />
		</ToolbarButton>
		<ToolbarButton :pressed="state.isStrikethrough" title="Strikethrough" @click="format('strikethrough')">
