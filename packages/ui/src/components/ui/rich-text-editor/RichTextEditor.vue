...
<template>
	<div :class="cn('rte-root flex flex-col rounded-md border border-input bg-background', props.class)">
		<LexicalComposer :initial-config="initialConfig">
			...
			<div class="rte-editor-shell relative" :style="{ '--rte-min-h': props.minHeight }">
				<RichTextPlugin>
					<template #contentEditable>
						<EditorContentEditable class="rte-editable prose prose-sm max-w-none px-4 py-3 focus:outline-none dark:prose-invert" :aria-label="props.ariaLabel" />
					</template>
					<template #placeholder>
						<div class="rte-placeholder pointer-events-none absolute left-4 top-3 text-sm text-muted-foreground">
							{{ props.placeholder }}
						</div>
					</template>
				</RichTextPlugin>
			...
			<HtmlSyncPlugin :model-value="props.modelValue" @update:model-value="(value) => emit('update:modelValue', value)" />
			<HistoryPlugin />
			<SelectionAlwaysOnDisplay />
			<ListPlugin />
			...
			<MarkdownShortcutPlugin v-if="resolvedFeatures.markdownShortcuts" :transformers="DEFAULT_TRANSFORMERS" />
			<TabIndentationPlugin />
			<TablePlugin v-if="resolvedFeatures.tables" :has-cell-merge="true" :has-cell-background-color="false" :has-tab-handler="true" :has-horizontal-scroll="true" />
			<HorizontalRulePlugin />
			<ImagePlugin v-if="resolvedFeatures.images" />
			<SlashMenuPlugin v-if="resolvedFeatures.slashMenu" :features="resolvedFeatures" @request-image="openImageDialog" />
		...
		<ImageUploadDialog v-if="resolvedFeatures.images" :open="showImageDialog" :upload="props.uploadImage" @update:open="(value) => (showImageDialog = value)" @insert="onInsertImage" />
	</div>
</template>
