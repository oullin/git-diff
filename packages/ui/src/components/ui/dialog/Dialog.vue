...
const focusInitialElement = (): void => {
  ...
  const firstTabbableElement = Array.from(
    dialogContent.querySelectorAll<HTMLElement>(tabbableSelector),
  ).find((element) => isFocusableElement(element));
...
<template>
  ...
  >
    <div
      class="fixed inset-0 z-50 flex min-h-full items-start justify-center overflow-y-auto px-4 py-6 sm:items-center sm:px-0"
      scroll-region
    >
      <transition
