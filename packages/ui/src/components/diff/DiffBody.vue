...
import { applyExpansions, parsePatch, splitPatchLines, type HunkInfo, type PatchLine, type SplitRow } from '@git-diff/domain/diff';

import { useLineSelection, type LineAnchor, type LineSelectionRange, type LineSide } from '@composables/useLineSelection';
...
function handleAddCommentClick(event: MouseEvent, section: DiffSection, line: PatchLine, explicitSide?: LineSide): void {
	...
	const target: LineAnchor = {
		...
		lineNumber: explicitSide === 'left' ? (line.oldLine ?? anchored.lineNumber) : explicitSide === 'right' ? (line.newLine ?? anchored.lineNumber) : anchored.lineNumber,
	};
		...
		<section v-for="section in file.sections" :key="section.id">
			<div data-diff-scroller="true" :style="{ overflowX: 'auto', overflowY: 'visible' }">
				<div :ref="setSectionRef(section.id)" :style="wrapperStyle">
					<SplitHandle v-if="viewMode === 'split'" :ratio="splitRatio" :container-el="sectionRefs[section.id] ?? null" @update:ratio="(value) => emit('update:splitRatio', value)" />
						...
						<template v-for="row in splitRows(section)" :key="row.id">
							...
							<template v-else-if="row.kind === 'context'">
								...
								>
									...
									<button type="button" class="gd-add-comment" title="Add comment (shift-click to extend selection)" @click="(e) => handleAddCommentClick(e, section, row.line)">
							...
							<template v-else>
								<DiffSplitRow
									...
									@add-comment="(e) => handleAddCommentClick(e, section, (row.right ?? row.left) as PatchLine)"
								/>
								<template v-for="line in [row.left, row.right].filter(Boolean) as PatchLine[]" :key="`${row.id}:${line.id}`">
									<CommentThread
						...
						<template v-for="line in patchLines(section)" :key="line.id">
							...
							<template v-else-if="line.type !== 'meta'">
								<div
									...
									:style="{
										background: bgFor(line.type === 'add' ? 'add' : line.type === 'del' ? 'rem' : 'ctx'),
										minHeight: `${lineH}px`,
									}"
								>
									<DiffGutter :num="line.oldLine ?? ''" :bg="numBgFor(line.type === 'add' ? 'add' : line.type === 'del' ? 'rem' : 'ctx')" />
									<DiffGutter
										:num="line.newLine ?? ''"
										:bg="numBgFor(line.type === 'add' ? 'add' : line.type === 'del' ? 'rem' : 'ctx')"
										:bar="line.type === 'add' || line.type === 'del' ? barFor(line.type === 'add' ? 'add' : 'rem') : undefined"
									/>
									<DiffCodeCell :html="highlightHtml(line.text)" :sign="sign(line.type === 'add' ? 'add' : line.type === 'del' ? 'rem' : 'ctx')" />
									<button type="button" class="gd-add-comment" title="Add comment (shift-click to extend selection)" @click="(e) => handleAddCommentClick(e, section, line)">
										<Plus :size="12" :stroke-width="2.5" />
									</button>
								</div>
