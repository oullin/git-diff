...
<template>
	<Dialog :show="open" max-width="2xl" @close="$emit('close')">
		...
		<DialogBody class="max-h-[70vh] overflow-auto">
			<div>
				<div :style="{ fontSize: '13px', fontWeight: 600, color: 'var(--gd-text)' }">Review notes</div>
				<p :style="{ marginTop: '4px', fontSize: '12px', color: 'var(--gd-text-3)' }">Local-only review timeline and rich comments.</p>
				<LazyRichTextEditor
					class="mt-3"
