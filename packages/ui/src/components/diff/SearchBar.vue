...
<template>
	<div v-if="props.open" class="fixed right-6 top-20 z-50 flex items-center gap-2 rounded-md border border-border bg-background/95 px-2 py-1.5 shadow-lg backdrop-blur" style="min-width: 22rem">
		<input ref="inputRef" v-model="query" class="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" type="text" placeholder="Find in diff…" @keydown="onKey" />
		...
		<button type="button" class="rounded p-1 hover:bg-muted" :disabled="matchCount === 0" @click="step(-1)" title="Previous (Shift+Enter)">
			<ChevronUp class="h-3.5 w-3.5" />
		</button>
		<button type="button" class="rounded p-1 hover:bg-muted" :disabled="matchCount === 0" @click="step(1)" title="Next (Enter)">
			<ChevronDown class="h-3.5 w-3.5" />
		</button>
		<button type="button" class="rounded p-1 hover:bg-muted" @click="emit('close')" title="Close (Esc)">
			<X class="h-3.5 w-3.5" />
		</button>
	</div>
</template>
