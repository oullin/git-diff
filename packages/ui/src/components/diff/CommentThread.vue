	...
	>
		...
		>
			<div class="flex items-center" :style="{ gap: '10px' }">
				...
				>
				<span :style="{ fontSize: '13.5px', fontWeight: 600, color: 'var(--gd-text)' }">{{ comment.authorLabel }}</span>
				<span :style="{ fontSize: '11.5px', color: 'var(--gd-text-3)' }">{{ relativeTime(comment.createdAt) }}</span>
				<span
					:style="{
						fontSize: '11px',
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end">
						...
						<DropdownMenuItem class="text-destructive" @select="emit('delete')"> Delete comment </DropdownMenuItem>
					</DropdownMenuContent>
				</DropdownMenu>
			</div>
			<div
			...
			>
				<LazyRichTextEditor v-model="draft" min-height="3rem" placeholder="Write a reply…" aria-label="Reply" :features="replyFeatures" />
