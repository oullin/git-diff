...
const squareKinds = computed<('g' | 'r' | 'n')[]>(() => {
	...
	return [...Array(g).fill('g' as const), ...Array(r).fill('r' as const), ...Array(n).fill('n' as const)];
});
...
<template>
	<span v-if="squares" class="inline-flex items-center" :title="`+${add} −${del}`" :style="{ gap: '2px' }">
		<span
	...
	<span v-else class="inline-flex items-center gap-1 font-mono font-semibold" :style="{ fontSize: mini ? '10.5px' : '11px' }">
