...
<template>
	<div class="gd-row relative" :style="{ display: 'grid', gridTemplateColumns: 'var(--gd-split-cols, 1fr 1fr)' }">
		<div
			class="flex"
			:style="{
		...
		>
			<DiffGutter :num="render.left.num" :bg="numBgFor(render.left.kind)" :bar="render.left.kind === 'rem' ? barFor('rem') : undefined" />
		...
		>
			<DiffGutter :num="render.right.num" :bg="numBgFor(render.right.kind)" :bar="render.right.kind === 'add' ? barFor('add') : undefined" />
			<DiffCodeCell :html="render.right.html" :sign="sign(render.right.kind)" />
		</div>
		<button v-if="canComment" type="button" class="gd-add-comment" title="Add comment (shift-click to extend selection)" @click="(e) => $emit('add-comment', e)">
