...
<template>
	<div class="flex flex-wrap items-center gap-2 border-b border-border bg-background/70 px-4 py-1.5 text-xs">
		<CommitPicker
			:commits="commits"
		...
		/>
		<span v-if="activePullRequest" class="flex items-center gap-1 font-mono text-muted-foreground">
			PR #{{ activePullRequest.number }}
			<span v-if="activePullRequest.title" class="font-sans">· {{ activePullRequest.title }}</span>
		</span>
		<span v-else-if="repoMode === 'commit' && state.commitSha" class="font-mono text-muted-foreground"> commit {{ state.branch || state.commitSha.slice(0, 7) }} </span>
		<span class="flex-1" />
		<button
			type="button"
