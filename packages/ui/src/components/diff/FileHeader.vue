...
const hasUnstaged = computed(() => props.file.sections.some((s: DiffSection) => s.kind === 'unstaged' || s.kind === 'untracked'));

const canPreview = computed(() => isMarkdownPath(props.file.path) && props.file.status !== 'deleted');
</script>
...
<template>
	...
	>
		...
		<div class="flex items-center min-w-0" :style="{ gap: '2px', fontFamily: 'var(--font-mono)', fontSize: '13.5px' }">
