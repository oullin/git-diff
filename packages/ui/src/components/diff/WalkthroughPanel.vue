...
import type { WalkthroughAction, WalkthroughGroup, WalkthroughImpact, WalkthroughRecord } from '@git-diff/domain';
	...
	<div v-if="error || record" class="border-b border-border bg-background/70 px-4 py-2 text-xs">
		...
		<div v-else-if="record" class="flex flex-col gap-2">
			...
			<div v-for="group in groups" :key="group.id" class="rounded border border-border/60">
				<button type="button" class="flex w-full items-center gap-2 px-2 py-1 text-left hover:bg-accent/40" @click="toggleGroup(group.id)">
					<span class="font-mono text-[10px] text-muted-foreground">
						{{ isCollapsed(group.id) ? '▶' : '▼' }}
					</span>
					<span class="font-medium">{{ group.title }}</span>
					<span class="ml-auto text-[11px] text-muted-foreground"> {{ group.files.length }} file{{ group.files.length === 1 ? '' : 's' }} </span>
				</button>
				<div v-if="!isCollapsed(group.id)" class="px-3 py-2">
					...
					<ol class="list-decimal pl-5 space-y-1">
						<li v-for="file in group.files" :key="file.path" class="flex flex-wrap items-baseline gap-1">
							<code class="font-mono text-[11px]">{{ file.path }}</code>
							<span class="ml-1 rounded px-1 py-px text-[10px] font-medium uppercase" :class="actionStyles[file.action] ?? actionStyles.review">
								{{ file.action }}
							</span>
							<span class="rounded px-1 py-px text-[10px] font-medium uppercase" :class="impactStyles[file.impact] ?? impactStyles.contained">
								{{ file.impact }}
							</span>
