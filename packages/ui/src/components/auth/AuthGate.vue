...
<template>
	<div v-if="mode === 'loading'" class="grid h-screen place-items-center bg-background text-sm text-muted-foreground">Loading…</div>
	<AuthSetup v-else-if="mode === 'setup'" :os-username="osUsername" @completed="onCompleted" />
	<AuthLogin v-else-if="mode === 'login'" :os-username="osUsername" @logged-in="onCompleted" @wiped="onWiped" />
	<slot v-else />
</template>
