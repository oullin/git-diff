...
import { PREF_KEYS, type ChangedFile, type FileSearchResult, type PullRequestSummary, type RepositoryState, type ReviewDetail, type ReviewSession } from '@git-diff/domain';
	...
	async function openRepo(path: string) {
		...
		await withBusy(async () => {
			try {
				...
				activeReview.value = reviews.value[0] ? await window.diffApp.reviewDetail(reviews.value[0].id) : null;
	...
	async function refresh() {
		...
		await withBusy(async () => {
			try {
				...
				const next = current.mode === 'commit' && current.commitSha ? await window.diffApp.readCommit(current.commitSha, current.root) : await window.diffApp.refreshRepository(current.root);
	...
	async function switchBranch(branch: string) {
		...
		await withBusy(async () => {
			try {
					...
					showToast(
						{
							...
							description: files.length > 0 ? `${files.length} file${files.length === 1 ? '' : 's'} would be overwritten by checkout: ${files.join(', ')}` : undefined,
						},
						0,
					);
				...
				onError(structured?.message ?? (cause instanceof Error ? cause.message : String(cause)));
			}
		});
	}
