...
export function useRepoSelectors(opts: RepoSelectorsOptions) {
    ...
    const selectedFile = computed<ChangedFile | null>(
        () => changedByPath.value.get(opts.selectedPath.value) ?? files.value[0] ?? null,
    );

    const selectedIsChanged = computed(
        () => !!opts.selectedPath.value && changedByPath.value.has(opts.selectedPath.value),
    );
    ...
    const userInitials = computed(() => {
        const name =
            opts.currentUser.value?.displayName ?? opts.currentUser.value?.osUsername ?? "GO";
    ...
    const changedIndex = computed(() =>
        files.value.findIndex((f) => f.path === opts.selectedPath.value),
    );
