...
export function useFileListFilter(opts: UseFileListFilterOptions) {
	...
	const filteredAllPaths = computed(() => {
		...
		return q ? opts.allPaths.value.filter((path) => path.toLowerCase().includes(q)) : opts.allPaths.value;
	});

	const progressPct = computed(() => (totalCount.value === 0 ? 0 : (viewedCount.value / totalCount.value) * 100));
