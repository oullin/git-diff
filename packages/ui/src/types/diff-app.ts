...
export interface DiffAppApi {
	...
	generateWalkthrough(request: { path?: string; kind?: RepositoryMode; sha?: string; refresh?: boolean }): Promise<WalkthroughRecord>;
	...
	listPendingComments(request: { path?: string; kind?: RepositoryMode; sha?: string }): Promise<{ comments: PendingComment[] }>;
	...
	readRepositoryFileRange(request: { root: string; path: string; ref?: string; startLine: number; endLine: number }): Promise<RepositoryFileRange>;
	...
	readRepositoryFileBytes(request: { root?: string; path: string; ref?: string }): Promise<{ data: Uint8Array; mime: string }>;
	...
	addReviewEvent(request: { reviewId: number; type: string; filePath?: string; message?: string; metadata?: string }): Promise<ReviewEvent>;
	...
	updateReviewComment(request: { reviewId: number; commentId: number; bodyHtml: string }): Promise<ReviewComment>;
	deleteReviewComment(request: { reviewId: number; commentId: number }): Promise<void>;
	setReviewCommentResolved(request: { reviewId: number; commentId: number; resolved: boolean }): Promise<ReviewComment>;
