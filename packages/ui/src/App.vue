...
const { file: selectedRepoFile, loading: selectedFileLoading, error: selectedFileError, load: loadSelectedFile, reset: resetSelectedFile } = useSelectedFile({ selectedPath });
...
<template>
	<AuthGate @entered="handleAuthCompleted" @wiped="handleAuthWiped">
		...
		>
			...
			<StatusBar v-if="tweaks.showStatusBar" :file-path="selectedFile?.path ?? ''" :viewed-count="files.filter((f) => isViewed(f)).length" :total="files.length" />
