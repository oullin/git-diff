...
class ElectronSettingsStore implements SettingsStore {
	...
	read(): Partial<RuntimeSettings> {
		try {
			const raw = JSON.parse(readFileSync(this.settingsPath(), 'utf8')) as Partial<RuntimeSettings>;
	...
	write(settings: Partial<RuntimeSettings>): void {
		...
		writeFileSync(this.settingsPath(), JSON.stringify(this.clean(settings), null, 2) + '\n', 'utf8');
	}
	...
	clean(settings: Partial<RuntimeSettings>): Partial<RuntimeSettings> {
		return {
			...
			workflowDbPath: settings.workflowDbPath?.trim() ? settings.workflowDbPath : this.defaultWorkflowDbPath(),
