...
const forbiddenScriptCommands = ['babel', 'biome', 'esbuild', 'eslint', 'jest', 'prettier', 'rollup', 'swc', 'terser', 'ts-jest', 'tsup', 'unbuild', 'webpack'];
...
function checkSourceImports(path: string): void {
	...
	const importPattern = /(?:from\s+["']([^"']+)["']|import\(\s*["']([^"']+)["']|require\(\s*["']([^"']+)["'])/gu;
...
function isForbiddenPackage(name: string): boolean {
	return forbiddenPackages.some((forbidden) => name === forbidden || name.startsWith(`${forbidden}/`));
}
