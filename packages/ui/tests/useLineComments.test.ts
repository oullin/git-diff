...
describe('lineSideAndNumber', () => {
	...
	test('falls back to left when only an old line exists', () => {
		expect(lineSideAndNumber(line({ type: 'context', oldLine: 3, newLine: undefined }))).toEqual({
			side: 'left',
			lineNumber: 3,
		});
	});
...
describe('useLineComments.commentsForLine', () => {
	...
	const { commentsForLine } = useLineComments({
		comments: () => [comment({ id: 1, side: 'right', lineNumber: 4 }), comment({ id: 2, side: 'left', lineNumber: 3 }), comment({ id: 3, filePath: 'other.ts', side: 'right', lineNumber: 4 })],
		filePath: () => 'a.ts',
	});
