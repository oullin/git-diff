...
function setup(overrides?: { state?: RepositoryState | null; selectedPath?: string; activeReview?: ReviewDetail | null; currentUser?: AuthUser | null }) {
...
describe('useRepoSelectors', () => {
	...
	test('selectedFile prefers the selected path, else the first file', () => {
		...
		expect(setup({ state: state(files), selectedPath: 'b.ts' }).selectedFile.value?.path).toBe('b.ts');
		expect(setup({ state: state(files), selectedPath: '' }).selectedFile.value?.path).toBe('a.ts');
	});
	...
	test('userInitials derives up to two uppercase initials', () => {
		expect(setup({ currentUser: { displayName: 'Ada Lovelace' } as AuthUser }).userInitials.value).toBe('AL');
