...
describe('matchesBinding', () => {
	...
	test('exact match', () => {
		...
		expect(matchesBinding(evt({ key: 'p', metaKey: true, shiftKey: true }), binding)).toBe(true);
	});
	...
	test('named keys (enter, escape, arrowdown)', () => {
		expect(matchesBinding(evt({ key: 'Enter', metaKey: true }), parseBinding('cmd+enter')!)).toBe(true);
