...
function controls(overrides: {
	...
	return useContextExpansionControls({
		...
		expandDown: (overrides.expandDown ?? vi.fn().mockResolvedValue(undefined)) as unknown as ExpandDownFn,
...
describe('canExpandDown', () => {
	...
	test('last hunk depends on downward EOF flag', () => {
		expect(controls({ downwardEof: false }).canExpandDown(SECTION, hunk({ isLast: true }))).toBe(true);
		expect(controls({ downwardEof: true }).canExpandDown(SECTION, hunk({ isLast: true }))).toBe(false);
	});
});
