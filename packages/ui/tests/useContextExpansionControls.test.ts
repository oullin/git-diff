...
function controls(overrides: {
    ...
    return useContextExpansionControls({
        ...
        expandUp: (overrides.expandUp ??
            vi.fn().mockResolvedValue(undefined)) as unknown as ExpandUpFn,
...
describe("canExpandDown", () => {
    ...
    test("last hunk depends on downward EOF flag", () => {
        expect(
            controls({ downwardEof: false }).canExpandDown(SECTION, hunk({ isLast: true })),
        ).toBe(true);
