...
describe('parseLaunchArgs', () => {
	...
	test('dev mode strips two argv entries (electron .)', () => {
		...
		try {
			const intent = parseLaunchArgs(['/path/to/electron', '/path/to/app', repo], false, '/elsewhere');
	...
	test('Electron internal flags are ignored', () => {
		const intent = parseLaunchArgs(['/bin/app', '--no-sandbox', '--remote-debugging-port=9222', 'abc1234'], true, '/repo');
