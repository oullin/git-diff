...
import { getAddedLineNumbers, isMarkdownPath, renderMarkdownWithLineAnchors } from '@lib/markdownPreview';

function makeFile(status: ChangedFile['status'], patch: string, overrides: Partial<ChangedFile> = {}): ChangedFile {
	return {
		path: 'README.md',
		status,
		additions: 1,
...
describe('getAddedLineNumbers', () => {
	test('returns the set of newLine numbers for added lines', () => {
		const patch = ['@@ -1,3 +1,5 @@', ' context 1', '+new 2', '+new 3', ' context 4', ' context 5'].join('\n');
