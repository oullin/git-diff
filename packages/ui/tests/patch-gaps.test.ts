...
import { applyExpansions, getHunkInfos, parseHunkHeader, parsePatch, splitPatchLines } from '@git-diff/domain/diff';
...
describe('applyExpansions', () => {
	test('splices expansion rows before the next hunk header in oldLine order', () => {
		...
		expect(ctxLines).toEqual(['context line a', 'context line c', 'gap line 13', 'gap line 14', 'context line 51']);
	});
...
describe('splitPatchLines', () => {
	test('treats expansion-derived context lines as single-cell context rows', () => {
		...
		const contextRows = rows.filter((r) => r.kind === 'context').map((r) => (r as { kind: 'context'; line: { text: string } }).line.text);
