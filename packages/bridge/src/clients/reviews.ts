...
export class ReviewClient {
	...
	create(request: Partial<ReviewSession>): Promise<ReviewSession> {
		return this.transport.request<ReviewSession>('POST', '/v1/reviews', request as Record<string, unknown>);
	}
	...
	addEvent(request: { reviewId: number; type: string; filePath?: string; message?: string; metadata?: string }): Promise<ReviewEvent> {
		return this.transport.request<ReviewEvent>('POST', `/v1/reviews/${request.reviewId}/events`, {
			type: request.type,
	...
	createComment(request: {
		...
		return this.transport.request<ReviewComment>('POST', `/v1/reviews/${request.reviewId}/comments`, {
			filePath: request.filePath,
	...
	updateComment(request: { reviewId: number; commentId: number; bodyHtml: string }): Promise<ReviewComment> {
		return this.transport.request<ReviewComment>('PATCH', `/v1/reviews/${request.reviewId}/comments/${request.commentId}`, { bodyHtml: request.bodyHtml });
	}

	deleteComment(request: { reviewId: number; commentId: number }): Promise<void> {
		return this.transport.request<void>('DELETE', `/v1/reviews/${request.reviewId}/comments/${request.commentId}`);
	}

	setCommentResolved(request: { reviewId: number; commentId: number; resolved: boolean }): Promise<ReviewComment> {
		return this.transport.request<ReviewComment>('PATCH', `/v1/reviews/${request.reviewId}/comments/${request.commentId}/resolve`, { resolved: request.resolved });
	}
}
