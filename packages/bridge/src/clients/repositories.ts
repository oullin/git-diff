...
export class RepositoriesClient {
	...
	searchFiles(request: { query: string; limit?: number }): Promise<{ results: FileSearchResult[] }> {
		...
		return this.transport.request<{ results: FileSearchResult[] }>('GET', `/v1/repositories/search-files?${params.toString()}`);
	}
	...
	remove(request: { path: string }): Promise<void> {
		return this.transport.request<void>('DELETE', `/v1/repositories?path=${encodeURIComponent(request.path)}`);
	}

	listCollaborators(request: { path: string }): Promise<{ collaborators: RepositoryCollaborator[] }> {
		return this.transport.request<{ collaborators: RepositoryCollaborator[] }>('GET', `/v1/repositories/collaborators?path=${encodeURIComponent(request.path)}`);
	}

	addCollaborator(request: { path: string; userId: number; role: 'write' | 'read' }): Promise<RepositoryCollaborator> {
		return this.transport.request<RepositoryCollaborator>('POST', '/v1/repositories/collaborators', {
			path: request.path,
	...
	removeCollaborator(request: { path: string; userId: number }): Promise<void> {
		...
		return this.transport.request<void>('DELETE', `/v1/repositories/collaborators?${params.toString()}`);
	}
}
