 ...
 */
export type BridgeErrorKind = 'transport' | 'auth' | 'validation' | 'notFound' | 'conflict' | 'server';
...
export interface HttpTransport {
	request<Response>(method: HttpMethod, path: string, body?: JsonBody, timeoutMs?: number): Promise<Response>;
...
export class SocketHttpTransport implements HttpTransport {
	...
	request<Response>(method: HttpMethod, path: string, body?: JsonBody, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS): Promise<Response> {
...
export function requestJson<Response>(socketPath: string, method: HttpMethod, path: string, body?: JsonBody, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS): Promise<Response> {
	return new Promise<Response>((resolve, reject) => {
		...
		const req = httpRequest({ socketPath, method, path, headers }, (res) => {
			consumeBody(res)
				.then((raw) => {
					if ((res.statusCode === 200 || res.statusCode === 201) && isJsonResponse(res)) {
						try {
							...
							fail(transportError(`${method} ${path} returned malformed JSON`, error));
						}
				...
				.catch((error: unknown) => fail(transportError(`${method} ${path} response read failed`, error)));
		});
		...
		req.on('error', (error: Error) => fail(transportError(`${method} ${path} transport error`, error)));
...
export function requestBytes(socketPath: string, path: string, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS): Promise<BytesResponse> {
	return new Promise<BytesResponse>((resolve, reject) => {
		...
		const req = httpRequest({ socketPath, method: 'GET', path, headers: { Accept: '*/*' } }, (res) => {
			consumeBytes(res)
				.then((data) => {
					...
					if (status === 200 || status === 201) {
						succeed({
							data,
							mime: (res.headers['content-type'] ?? 'application/octet-stream') as string,
						});
				...
				.catch((error: unknown) => fail(transportError(`GET ${path} response read failed`, error)));
		});
		...
		req.on('error', (error: Error) => fail(transportError(`GET ${path} transport error`, error)));
...
function buildHttpError(method: HttpMethod, path: string, res: IncomingMessage, raw: string): BridgeError {
	const status = res.statusCode ?? 0;
