...
func NewAuthService(users *storage.UserRepo, sessions *storage.SessionRepo, cfg AuthConfig) *AuthService {
	if cfg.BcryptCost == 0 {
...
func (s *AuthService) Setup(
	...
	user, err := s.users.GetUserByOSUsername(ctx, osUsername)
	...
	_ = s.users.TouchUserLogin(ctx, user.ID)
...
func (s *AuthService) Wipe(ctx context.Context, activeOSUsername, requestedOSUsername string) error {
	...
	user, err := s.users.GetUserByOSUsername(ctx, target)

	if err != nil {
		return fmt.Errorf("read user: %w", err)
	}

	if err := s.users.WipeUser(ctx, user.ID); err != nil {
		return fmt.Errorf("wipe user: %w", err)
	}

	if _, err := s.users.EnsureUser(ctx, target); err != nil {
